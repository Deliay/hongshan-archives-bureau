        ...
        return (
          <details key={id} className="border border-archive-border rounded bg-archive-file">
            <summary className="px-3 py-2 text-sm text-archive-ivory cursor-pointer hover:text-archive-gold transition-colors font-mono">
              {id}
              <span className="text-archive-dust ml-2 text-xs">{translate(locale, 'diff.dataBundleCount', { count: bundle.length })}</span>
            </summary>
...
function ChangedList({ entries, locale }: { entries: Record<string, any>; locale: string }) {
  const keys = Object.keys(entries)
  if (keys.length === 0) return <p className="text-sm text-archive-lead">{translate(locale, 'common.empty')}</p>
  return (
    <div className="space-y-2">
    ...
    <div className="border border-archive-border rounded p-2 bg-archive-ink">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <span className="text-xs text-archive-dust ml-2">{translate(locale, 'common.level', { level: bundle.level })}</span>
          {bundle.iconId && <span className="text-xs text-archive-lead ml-2">{bundle.iconId}</span>}
        </div>
        <div className="text-xs text-archive-lead">
          {bundle.costType && <span>{translate(locale, 'diff.cost', { type: bundle.costType, value: bundle.costValue })}</span>}
      ...
      {bundle.blackboard && bundle.blackboard.length > 0 && (
        <div className="mt-1">
          <div className="text-[10px] text-archive-lead">{translate(locale, 'diff.params', { count: bundle.blackboard.length })}</div>
