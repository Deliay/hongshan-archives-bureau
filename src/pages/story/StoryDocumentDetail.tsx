  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      {(item.type === 'text' || item.type === 'document') && (
        <div>
          {item.contents.length === 0 && !loading && (
      ...
      {item.type === 'multi_media' && item.script && (
        <div>
          ...
          <div className="space-y-3">
            {item.script.map((entry, i) => (
              <div key={i} className="flex gap-3">
                <span className="font-medium text-archive-gold shrink-0">{entry.speaker}</span>
                <span className="text-sm text-archive-ivory">{entry.line}</span>
              </div>
            ))}
          </div>
        </div>
