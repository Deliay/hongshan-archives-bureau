...
export default function RaceList() {
  ...
  if (!races) return <div className="text-[#8B8982] text-sm">暂无记录</div>

  const nonEmpty = races.filter(r => r.members.length > 0)
  if (nonEmpty.length === 0) return <div className="text-[#8B8982] text-sm">暂无记录</div>
        ...
        {nonEmpty.map((race) => (
          ...
          >
            <div className="flex items-baseline gap-2 mb-3">
              <h3 className="text-base font-medium text-[#E8E6E3] group-hover:text-[#C9A96E] transition-colors">{race.name}</h3>
