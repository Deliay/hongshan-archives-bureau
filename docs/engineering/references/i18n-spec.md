...
2. 若是站点自定义文案，在 `CUSTOM` 中新增 key。
3. 每个 `CUSTOM` key 必须提供全部 14 个语言（CN/TC/EN/JP/KR/RU/MX/BR/DE/FR/VN/TH/ID/IT）的本土翻译，不允许使用任何语言占位、不允许留空、不允许直接复制中文。
4. 运行 `node scripts/generate-i18n-dicts.ts` 重新生成字典。
5. 运行 `npm run lint && npm run test && npm run build` 验证。
