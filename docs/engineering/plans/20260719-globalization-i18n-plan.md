...
> **重要**：本文件中的 `import` 列表与 `messages` 映射由 `scripts/generate-i18n-dicts.ts` 根据 `GET /i18n` 返回的可用 locale 列表自动生成，不应手动维护。以下示例仅展示部分代表 locale，实际生成以 `/i18n` 返回的全部 locale 为准（当前为 14 个）。
...
下表展示部分高频 key 的多语言译文（示例仅列出 6 个代表 locale，完整字典覆盖 `/i18n` 返回的全部 locale），用于验证翻译来源与风格一致性。
...
| 风险 | 影响 | 缓解措施 |
|------|------|----------|
| 字典体积过大 | 构建产物增长 | 一次性引入 `/i18n` 返回的全部 14 种语言，未提供自定义译文的 locale 自动回退到 EN/CN |
