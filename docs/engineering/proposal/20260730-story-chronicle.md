...
- `resolveDialog`：线性遍历、分支切换后后续消息更新、切换后丢弃旧选择、环保护、表情回应归并、未知 contentType 跳过。
- `adaptBakerChat`：chatType → 干员/联系人/群聊映射，speaker 名称/头像解析。
...
- 文库页页签切换、卷展开、条目跳详情。
- 详情页正文与剧本渲染、图片加载。
- Baker：Tab 筛选、选择会话加载消息、切换分支后消息更新、图片/表情包/表情回应展示。
