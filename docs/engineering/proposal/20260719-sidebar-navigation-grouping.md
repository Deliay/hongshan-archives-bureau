...
- 改造 `src/routes/ArchiveHome.tsx`，按分组卡片展示模块入口；
...
1. 将 `ArchiveHome.tsx` 的 `MODULES` 改造为 `MODULE_GROUPS`；
2. 每个分组渲染为一个 `Card`，顶部显示分组名称与描述，底部列出模块链接；
3. 将「职业与属性」归入「人事档案」分组。
