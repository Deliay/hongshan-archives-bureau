...
宏山档案馆是我在《明日方舟：终末地》旅途中所积累的资料集。以游戏数据为基础，为我所见的塔卫二留下结构化的记录。

**我的角色**：终末地管理员，数据的整理者
**内容来源**：游戏内数据文件、剧情文本、我的见闻
**核心价值**：准确、结构化、可追溯
...
| 元素 | 说明 |
|------|------|
| Logo | 「宏山档案馆」文字 + 图标，副标题「我的档案」 |
| 导航链接 | 我的各卷档案入口（下拉可展开子卷） |
| 搜索框 | 全局搜索，聚焦时弹出搜索面板 |
| 主题切换 | 明/暗模式切换 |
...
- **详情页**：大立绘（可点击放大）+ 基础信息区 + 属性面板 + 档案/语音标签页 + 关联武器/剧情/同阵营
- **特色**：职业图标 + 属性色作为卡片视觉标识；筛选器放在页面顶部固定位置

### 02 我的武器

- **列表页**：按武器类型分组折叠，或全部平铺（模型渲染图 + 名称 + 星级）
- **详情页**：武器模型图 + 属性面板 + 技能/天赋 + 故事文本（独立区域，如「嵌合正义」全文展示）+ 适用干员
- **特色**：故事文本使用类似 PRTS 的富文本样式，区别于普通属性区域
...
- **装备详情**：属性 + 套装归属 + 获取途径
- **套装详情**：套装名称 + 2件套效果 + 4件套效果 + 适用干员推荐
- **宝石列表**：词条池 + 可镶嵌部位 + 强化/重铸消耗

### 09 道具材料

- **列表页**：按类型分组（材料/消耗品/收集品/礼物）+ 网格卡片
- **详情页**：图标 + 描述 + 获取途径（关卡/商店/工厂）+ 用途（用于哪些合成）
- **特色**：用途倒查功能（点击材料查看所有使用到此材料的配方）
...
- **总览页**：时间线 + 分类入口（PRTS 文献 / 剧情对话 / SNS / 广播）+ 搜索
- **文献详情**：富文本全文展示 + 关联角色/地区/敌人 + 前后文导航
