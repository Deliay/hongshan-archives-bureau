...
- **描述**: 物品的基础展示单元为正方形方块，核心由物品图标与稀有度表达构成。规则如下：
  ...
  - 稀有度色条位于图标下方，为方块的标准组成部分。
  - 图标加载失败时显示占位底色，不出现破图。
- **用户价值**: 用户在任何页面看到物品时，能凭借一致的视觉迅速识别物品与稀有度。
- **验收标准**:
  - [ ] 物品方块在所有场景均为正方形。
...
- **描述**: 物品方块提供标准化的信息挂载位置，各场景按需启用，未启用的槽位不占位：
  ...
  | 名称 | 方块下方，最多两行截断 | 列表网格默认展示；密集场景可隐藏 |
  | 类型标签 | 名称下方的弱提示小字 | 武器类型、物品分类、装备部位 |
  | 稀有度星级 | 以 ★ 表达的稀有度 | 详情页头部等强调场景 |
...
- **描述**: 武器图鉴与装备图鉴的列表条目改为横向长条卡片：左侧为正方形物品方块，右侧为属性展示区，条目整体可点击进入对应卷宗。
  - 武器长条右侧：名称、稀有度、武器类型、技能名称。
  - 装备长条右侧：名称、稀有度、部位、套装归属、核心属性摘要。
