...
- **描述**: 左右结构布局。左侧为物品列表，收录所有参与机器制造的物品——即至少在一个机器配方中作为材料或作为产物出现的物品；列表项以统一物品方块展示（图标、名称、稀有度），支持按名称搜索。右侧为配方区：点击左侧物品后，展示该物品的全部相关配方，分为「作为产物」与「作为材料」两组；未选中物品时右侧展示引导提示。选中的列表项有高亮态，刷新/分享地址后仍定位到该物品。
- **用户价值**: 以物品为中心一站式查询「怎么来、怎么用」，覆盖资料查询与材料反查两类高频场景。
- **验收标准**:
  - [ ] 左侧列表完整覆盖所有参与机器制造的物品（既含作为材料的，也含仅作为产物的）。
  - [ ] 左侧列表支持按名称搜索过滤。
  - [ ] 点击物品后，右侧按「作为产物」「作为材料」分组展示全部相关配方。
  - [ ] 仅作为材料（或仅作为产物）的物品，对应分组展示为空态提示而非缺失。
...
| 工厂配方 | 左右结构：左物品列表，右配方区 | 物品方块列表、搜索框、「作为产物」「作为材料」分组、配方卡片 |
