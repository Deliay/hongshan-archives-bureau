...
- **描述**: 在种族详情页与阵营详情页的「相关记载」标题同一行末尾，增加一个穿透链接。点击后跳转至 `/archive/search?keyword={raceName|factionName}`，在全局搜索页查看该关键词的全部相关记载。
- **用户价值**: 从卷宗关联信息自然过渡到全局搜索，扩大信息覆盖面。
- **验收标准**:
  - [ ] 「相关记载」行尾出现可点击的搜索入口（文案/图标形式）。
...
| 种族详情页 | 展示种族资料与相关记载 | 种族名、相关记载行尾穿透链接、成员列表 |
| 阵营详情页 | 展示阵营资料与相关记载 | 阵营名、相关记载行尾穿透链接、成员列表 |
