...
现有导航中的「敌人图鉴」未落入上述四个业务领域。建议采用以下方案之一：

- **方案 A（推荐）**：新增「威胁档案」分组，专门收录敌人图鉴；未来可扩展为「敌对势力」「威胁等级」等子模块。
- **方案 B**：将敌人图鉴暂置于「地理档案」分组下，作为「区域威胁」视角的补充。
- **方案 C**：维持敌人图鉴为独立导航项，不参与本次分组。

> 待 Review 确认：本方案默认采用方案 A，即在四个分组之外新增「威胁档案」分组。如采纳方案 B 或方案 C，需在后续版本中补充说明。
...
该顺序基于管理员日常查阅频率排列：干员与物资为最高频，地理与敌人为场景补充，大事记为低频但重要的记录入口。
...
- 分组标签使用档案编号字体或正文无衬线字体，字号 11–12px；
- 分组标签文字可使用全大写或加字间距，增强档案索引感；
- 分组之间使用 16–24px 垂直间距，分组内入口保持 4–8px 间距；
- 可考虑在分组标签前添加极细竖线或圆点装饰，强化分类边界，但需保持克制。
...
- [ ] 侧边导航展示「人事档案」「物资档案」「地理档案」「大事记」四个分组；
...
- [ ] 敌人图鉴的归属方案经 Review 确认并落地。

## 待确认问题

1. **敌人图鉴归属**：是否新增「威胁档案」分组？或采用方案 B/C？
2. **分组命名**：「大事记」是否保留，或改为「记录档案」「纪事档案」等更档案局风格的名称？
3. **分组顺序**：当前顺序是否满足查阅习惯，是否需要 A/B 验证？
4. **视觉表现**：分组标签是否需要装饰线或图标，还是仅使用纯文本？
