---
description: 剧情纪事模块重构：剧情回顾长卷、PRTS 馆藏文库与 Baker 聊天终端
type: Fleeting
---
...
**v1.1 变更**: 根据 review 意见新增「Baker」聊天终端模块（功能点 5）。
...
将 `/archive/story` 重构为「剧情纪事」模块，包含一个总览页与两大内容板块：「纪事长卷」（剧情梗概连续阅读）与「馆藏文库」（PRTS 文献分类浏览与全文阅读）。同时在「大事记」分组下新增独立模块「Baker」（`/archive/baker`），还原游戏内聊天软件的会话剧情。
...
命名理由：「纪事」呼应所在分组「大事记」（chronicle），点明模块叙事属性；「长卷」传达连续通读的体验；「馆藏文库」呼应档案馆设定，且六类文献本身即游戏内 PRTS 系统的馆藏分类。「Baker」为游戏内聊天软件的官方名称（各语言均称 Baker），直接使用以保证官方感。
...
- **描述**: 在「大事记」分组下新增「Baker」模块（`/archive/baker`），完整收录游戏内聊天软件 Baker 的会话剧情。页面采用聊天软件式双栏布局：**左侧为联系人列表**，支持「全部 / 干员 / 联系人 / 群聊」四个 Tab 筛选（干员=干员单聊、联系人=NPC 单聊、群聊=多人会话）；**右侧为聊天界面**，按会话（一场聊天）为单位展示消息流，多场会话按剧情发生顺序以分隔条串连。聊天中出现的**分支选项**以选项组形式呈现：默认展示第一个选项的走向，管理员可切换任一选项，切换后该选项成为「我」发出的消息，后续消息按照切换后的分支重新展示（聊天整体为固定路径，分支最终汇合）。消息类型支持：**文本**、**图片**、**表情包**（含以表情包回复的选项）、**表情回应**（pin reaction，以角标形式附着在对应消息上，展示表情与回应人）、**系统提示**（如「xx已离线」，居中展示）、**PRTS 收藏分享**与**任务链接**（以卡片形式展示）。群聊中每条他人消息展示发送者头像与昵称；「我」（管理员）的消息靠右展示。
