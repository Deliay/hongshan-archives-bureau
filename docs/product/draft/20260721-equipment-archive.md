...
档案馆已上线干员、武器、道具、敌人等图鉴模块，但「装备」模块目前仅有占位页。装备是终末地工业体系中干员养成的核心一环：装备分为身体、护手、挂件三个部位，高阶装备隶属于不同「套组」，集齐套组可激活套装技能；装备可通过配方制作，也可通过消耗同部位金色品质装备进行「精锻」强化。馆员目前无法在任何页面查阅装备的属性、套装效果、精锻材料与制作配方。
...
| 查询属性 | 玩家/攻略作者 | 查看装备主属性、副属性及精锻后的强化数值 | 站外资料零散且多语言不全 |
...
- **描述**: 卷宗页展示装备名称、图标、稀有度、部位、最低穿戴等级、物品描述与背景文本；属性区展示主属性与全部副属性，并展示每条副属性精锻强化后的数值。
- **用户价值**: 一页看清单件装备的全部数值，包括强化上限。
- **验收标准**:
  - [ ] 头部展示图标、名称、稀有度色条、部位与穿戴等级。
  - [ ] 属性区展示主属性与副属性名称及数值；有强化值的副属性展示各强化阶段数值。
  - [ ] 描述与背景文本富文本正常解析。
...
- **描述**: 卷宗页展示可用于精锻当前装备的其他装备清单。依据游戏机制：精锻需消耗与待精锻装备**同部位**的**金色品质**装备。列表按稀有度与穿戴等级排序，标注每件材料的图标与名称，可点击跳转。
- **用户价值**: 强化前即可规划材料来源，无需在游戏内逐个翻找。
- **验收标准**:
  - [ ] 列出全部同部位且金色品质的其他装备（排除装备自身）。
  - [ ] 材料卡片可跳转至对应装备卷宗。
  - [ ] 无可用材料时（如装备本身非金色品质的低阶部位且无同部位金装）展示空态提示。
