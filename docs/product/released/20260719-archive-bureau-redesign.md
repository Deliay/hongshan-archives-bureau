...
- **验收标准**:
  - [x] 主背景色为深沉墨黑，减少屏幕反光感
  - [x] 强调色使用低饱和沉金，用于核心标识、hover 与激活态
  - [x] 引入印章红作为高亮、警告、版本差异标识的辅助色
  - [x] 文字层级至少包含正文、次级、禁用三个明度，保证可读性
...
- **验收标准**:
  - [x] 着陆页、首页大标题使用衬线风格字体
  - [x] 卷宗编号、档案标签使用等宽或窄体字形
  - [x] 同一层级的标题与正文在字重、字号、颜色上保持一致
  - [x] 中文行高不小于 1.6，长文本使用宽松行距
...
- **验收标准**:
  - [x] 桌面端内容区最大宽度限制在合理范围，避免过宽导致阅读疲劳
  - [x] 首页每个卷宗入口包含：卷宗编号、名称、描述、状态/子入口
  - [x] 列表页包含档案编号列或编号徽章
  - [x] 详情页顶部包含卷宗头（名称、编号、密级/分类、立绘/头像）
...
- **验收标准**:
  - [x] 侧边栏顶部展示档案局徽章或印章式标识
  - [x] 卷宗卡片与详情页展示编号前缀
  - [x] 徽章系统包含：公开、受限、核心等档案级别（如适用）
  - [x] 装饰元素不过度，保持克制
...
- **验收标准**:
  - [x] 着陆页过渡时长控制在 500–800ms，缓动自然
  - [x] 卡片 hover 仅有边框/背景/微抬升一种变化
  - [x] 页面内容加载使用统一的骨架屏，风格与新版一致
  - [x] 尊重系统「减少动效」偏好
...
- **验收结果**: 已通过。`npm run build`、`npm run test`、`npm run lint` 与 Playwright E2E（44 项）全部通过。
- **已知遗留**: `oxlint` 仍有 26 个既有警告（非本次改动引入），后续逐步处理。
