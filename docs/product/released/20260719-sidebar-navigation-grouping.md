...
本次优化针对左侧导航栏与档案局首页的信息组织方式，将现有入口按业务领域重新聚类，使导航从「单级长列表」升级为「分组索引」，首页从「模块平铺」升级为「分组卡片」。
...
该顺序基于管理员日常查阅频率与作战关联性排列：人事与威胁为高频核心档案，物资与地理为场景补充，大事记为低频但重要的记录入口。
...
- [x] 侧边导航展示「人事档案」「威胁档案」「物资档案」「地理档案」「大事记」五个分组；
- [x] 「人事档案」下包含：干员档案、干员种族、干员阵营；
- [x] 「威胁档案」下包含：敌人图鉴；
- [x] 「物资档案」下包含：道具材料、武器档案、装备系统、工厂系统；
- [x] 「地理档案」下包含：地区地理；
- [x] 「大事记」下包含：剧情记录、更新日志；
- [x] 分组标签为纯展示元素，不可点击、不可折叠/展开；
- [x] 分组内入口的激活态、hover 态与现有行为保持一致；
- [x] 移动端抽屉导航应用相同的分组规则；
- [x] 多语言场景下分组标签文案正常切换，无截断或错位；
- [x] 档案局首页按相同分组展示模块入口，点击可正常跳转。
...
- **验收结果**: 已通过。`npm run build`、`npm run test`、`npm run lint` 与 Playwright E2E 全部通过。
- **关键决策**:
  - 新增「威胁档案」分组，收录敌人图鉴；
  - 分组标签采用纯文本 uppercase 风格；
  - 档案局首页同步按分组展示；
  - 「职业与属性」不纳入本次分组，保持独立入口。
