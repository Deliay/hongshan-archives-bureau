...
test.describe('档案搜索 (Archive Search)', () => {
  ...
  test('侧边导航可进入档案搜索页', async ({ page }) => {
    ...
    await expect(page.locator('h2').first()).toHaveText('搜索结果', { timeout: 15000 })
  })
  ...
  test('输入关键词后回车触发搜索', async ({ page }) => {
    ...
    const input = page.getByPlaceholder('搜索干员、武器、道具…')
  ...
  test('切换语言后搜索页标题和占位符跟随变化', async ({ page }) => {
    ...
    await expect(page.locator('h2').first()).toHaveText('Search Results', { timeout: 15000 })
    await expect(page.getByPlaceholder('Search operators, weapons, items…')).toBeVisible({ timeout: 5000 })

    // Switch to FR
    await setLocale(page, 'FR')
    await expect(page.locator('h2').first()).toHaveText('Résultats de recherche', { timeout: 15000 })
    await expect(page.getByPlaceholder('Rechercher opérateurs, armes, objets…')).toBeVisible({ timeout: 5000 })
  })
