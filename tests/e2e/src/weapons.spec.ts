...
test.describe('武器档案 (Weapon Archive)', () => {
  ...
  test('武器列表显示所有武器', async ({ page }) => {
    ...
    await expect(page.getByRole('main').getByText('武器档案')).toBeVisible({ timeout: 5000 })
    const card = page.locator('main a[href*="/archive/weapons/"]').first()
    await expect(card).toBeVisible({ timeout: 15000 })
    const count = await page.locator('main a[href*="/archive/weapons/"]').count()
  ...
  test('武器类型筛选', async ({ page }) => {
    ...
    if (firstType) {
      ...
      const cardCount = await page.locator('main a[href*="/archive/weapons/"]').count()
      expect(cardCount).toBeGreaterThan(0)
    }
  })

  test('武器详情显示技能信息', async ({ page }) => {
